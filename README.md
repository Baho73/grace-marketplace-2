...
| `grace-evolve` | Evolutionary / comparative search over solution variants with worktree isolation |
...
| `src/grace-afk.ts`, `src/afk/*` | `grace afk` subcommand tree (start/tick/ask/check/journal/defer/increment/report/stop) and its building blocks — session state, journal, config, Telegram transport |
| `src/grace-evolve.ts`, `src/evolve/*` | `grace evolve` subcommand tree (init/run) — spec, orchestrator, scoring, worktree isolation |
| `src/lint/*` | `grace lint` implementation (+ SKILL.md section rule) |
| `src/query/*` | Projection-backed query layer for CLI navigation |
| `docs/knowledge-graph.xml`, `docs/development-plan.xml`, `docs/verification-plan.xml` | Self-governance — this repo is GRACE-managed (GRACE 3 layout, pending `$grace-migrate`) |
